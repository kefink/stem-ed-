...
export default function LoginPage() {
  ...
  const completeSignIn = async (loginData: any) => {
  ...
  const handleTwoFactorSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/v1/auth/login/two-factor", {
        method: "POST",
      ...
      if (!res.ok) {
        const detail =
          data?.detail || data?.message || "Invalid authentication code";
        setTwoFactorError(detail);
      ...
      if (!result.success) {
        setTwoFactorError(result.error || "Sign-in failed. Please try again.");
        setIsSubmitting(false);
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loginData?.requires_two_factor && loginData?.challenge_token) {
        setTwoFactor({
          challengeToken: loginData.challenge_token,
          message:
            loginData.message ||
            "Two-factor authentication is required to complete your sign-in.",
          backupCodesRemaining: loginData.backup_codes_remaining ?? null,
        });
        return;
      }

      const result = await completeSignIn(loginData);
      if (!result.success) {
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          {twoFactor && (
            <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg mb-4">
              ...
              {typeof twoFactor.backupCodesRemaining === "number" && (
                <p className="text-sm text-blue-600 mt-2">
                  Backup codes remaining: {twoFactor.backupCodesRemaining}
                </p>
              )}
            </div>
          )}
